...
{ 
    ...
    if ( [[NSUserDefaults standardUserDefaults] boolForKey:@"virtualBox"] ) {
        // Send CTRL + V (For Windows VM Machines)
        NSNumber *keyCodeCtrl = [srTransformer reverseTransformedValue:@"V"];
        CGKeyCode veeCodeCtrl = (CGKeyCode)[keyCodeCtrl intValue];
        CGPostKeyboardEvent( (CGCharCode)0, (CGKeyCode)0x3B, true ); // Ctrl down
        CGPostKeyboardEvent( (CGCharCode)'v', veeCodeCtrl, true ); // V down
        CGPostKeyboardEvent( (CGCharCode)'v', veeCodeCtrl, false ); //  V up
        CGPostKeyboardEvent( (CGCharCode)0, (CGKeyCode)0x3B, false ); // Ctrl up
    }
}
